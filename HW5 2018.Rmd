...
load("/Users/Lacey/Desktop/N741/2018Week8/HMPv35.RData")
...
alpha_meas = c("Observed", "Chao1", "ACE", "Shannon", "Simpson", "InvSimpson")
(p2 <- plot_richness(HMPv35sub2, "HMPbodysubsite", measures=alpha_meas))

p2 + geom_boxplot(data=p$data, aes(x=HMPbodysubsite, y=value, color=NULL), alpha=0.1)
...
#Filter out taxa with low occurrence throughout all samples from the HMPv35sub2 object

HMPv35subsub2 = filter_taxa(HMPv35sub2, function(x) sum(x > 3) > (0.2*length(x)), TRUE)

# The next step filters out all taxa that occur in less than .01% of samples

HMPv35sub2r  <- transform_sample_counts(HMPv35subsub2, function(x) x / sum(x) )
...
title = "plot_bar; by site; Bacteroidetes only for HMPv35sub2frbac"
...
#I like the first method of display better because it is easier to distinguish between the colors than the shapes. In this case, I think it is more important to be able to visually discriminate between body sites than between sexes.

```

#### Link for homework 
[https://github.com/lpgleason/2018Week8.git](https://github.com/lpgleason/2018Week8.git)
